...
use bonnie_lib::{get_command_from_cfg_and_args, get_cfg_path, get_cfg, run_cmd, init, help};
...
fn main() {
    ...
    if prog_args.get(1) == Some(&String::from("help")) {
        ...
        let output = init();
        match output {
            Ok(_) => return println!("Bonnie configuration file created at './bonnie.toml'. Enjoy!"),
